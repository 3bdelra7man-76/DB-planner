...
export const CARDINALITIES: RelationCardinality[] = [
  'one-to-one',
  'one-to-many',
  'many-to-one',
  'many-to-many',
...
export function touchSchema(schema: SchemaDocument): SchemaDocument {
  return {
    ...schema,
    project: {
      ...schema.project,
...
export function sourceHandleId(fieldId: string): string {
  return `source:${fieldId}`;
}

export function targetHandleId(fieldId: string): string {
  return `target:${fieldId}`;
}

export function parseFieldHandle(handleId?: string | null): string | null {
  if (!handleId) {
    return null;
  }
...
export function createTable(name: string, position: XYPosition): SchemaTable {
...
export function createIndex(fields: string[]): SchemaIndex {
  return {
    id: createId('index'),
    name: 'idx_new',
    fields,
...
export function createEnum(position: XYPosition): SchemaEnum {
  return {
    id: createId('enum'),
    name: 'status',
...
export function createNote(position: XYPosition): SchemaNote {
  return {
    id: createId('note'),
    title: 'Design note',
...
export function createStarterSchema(): SchemaDocument {
    ...
    tables: [
      ...
      },
      {
        ...
        fields: [
          ...
          createField({ id: postAuthorId, name: 'author_id', type: 'uuid', foreignKey: true, required: true }),
          createField({ id: postCategoryId, name: 'category_id', type: 'uuid', foreignKey: true, required: true }),
...
export function createTemplateSchema(template: 'auth' | 'commerce' | 'content'): SchemaDocument {
  if (template === 'content') {
    return {
      ...createStarterSchema(),
      project: {
...
function createAuthTemplate(): SchemaDocument {
  return {
    version: 1,
    project: {
      name: 'Auth Schema',
    ...
    tables: [
      ...
      },
      {
        ...
        fields: [
          ...
          createField({ id: 'field_auth_profiles_user_id', name: 'user_id', type: 'uuid', foreignKey: true, required: true, unique: true }),
      ...
      },
      {
        ...
        fields: [
          ...
          createField({ id: 'field_auth_sessions_user_id', name: 'user_id', type: 'uuid', foreignKey: true, required: true }),
...
function createCommerceTemplate(): SchemaDocument {
  return {
    version: 1,
    project: {
    ...
    tables: [
      ...
      },
      {
        ...
        fields: [
          ...
          createField({ id: 'field_shop_orders_customer_id', name: 'customer_id', type: 'uuid', foreignKey: true, required: true }),
      ...
      },
      {
        ...
        fields: [
          ...
          createField({ id: 'field_shop_items_order_id', name: 'order_id', type: 'uuid', foreignKey: true, required: true }),
          createField({ id: 'field_shop_items_product_id', name: 'product_id', type: 'uuid', foreignKey: true, required: true }),
...
export function normalizeSchema(value: unknown): SchemaDocument {
  if (!isRecord(value)) {
...
export function validateSchema(schema: SchemaDocument): ValidationIssue[] {
...
function normalizeTable(value: unknown, index: number): SchemaTable {
...
function normalizeField(value: unknown, index: number): SchemaField {
  ...
  return {
    ...
    type: fieldTypeSet.has(rawType as FieldType) ? (rawType as FieldType) : 'string',
    primaryKey: readBoolean(record.primaryKey, false),
    foreignKey: readBoolean(record.foreignKey, false),
...
function normalizeIndex(value: unknown, index: number): SchemaIndex {
  const record = isRecord(value) ? value : {};

  return {
    id: readString(record.id, createId('index')),
    name: readString(record.name, `idx_${index + 1}`),
    fields: readArray(record.fields).filter((fieldId): fieldId is string => typeof fieldId === 'string'),
    unique: readBoolean(record.unique, false),
  };
}

function normalizeRelation(value: unknown, index: number): SchemaRelation {
  const record = isRecord(value) ? value : {};
  ...
  return {
    ...
    cardinality: cardinalitySet.has(cardinality as RelationCardinality) ? (cardinality as RelationCardinality) : 'many-to-one',
    onDelete: deleteActionSet.has(onDelete as DeleteAction) ? (onDelete as DeleteAction) : 'restrict',
    label: readString(record.label, `relation_${index + 1}`),
  };
}

function normalizeEnum(value: unknown, index: number): SchemaEnum {
  const record = isRecord(value) ? value : {};

  return {
    id: readString(record.id, createId('enum')),
    name: readString(record.name, `enum_${index + 1}`),
    values: readArray(record.values).filter((item): item is string => typeof item === 'string'),
    position: normalizePosition(record.position, { x: 160 + index * 300, y: 500 }),
  };
}

function normalizeNote(value: unknown, index: number): SchemaNote {
  const record = isRecord(value) ? value : {};
...
function normalizePosition(value: unknown, fallback: XYPosition): XYPosition {
  if (!isRecord(value)) {
    return fallback;
  }
